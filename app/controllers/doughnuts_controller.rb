class DoughnutsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @doughnut.save
        ...
        format.json { render action: 'show', status: :created, location: @doughnut }
      else
  ...
  def destroy
    @doughnut.destroy
    respond_to do |format|
      format.html { redirect_to doughnuts_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def doughnut_params
      params.require(:doughnut).permit(:name, :water, :sugar, :salt, :eggs, :yeast, :filling)
    end
end
